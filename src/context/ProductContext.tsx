  ...
  const refreshOrders = useCallback(async () => {
    ...
    try {
      ...
      const { data: rows, error } = await supabase
        .from('orders')
        .select('order_id, customer_name, phone, total_amount, status, created_at, items, products_json, customer, address, city, notes, payment_reference, payment_proof_url, shipping_tier, courier')
        .order('created_at', { ascending: false });
